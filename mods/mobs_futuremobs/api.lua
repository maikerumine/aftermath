futuremobs = {}
function futuremobs:register_mob(name, def)
	minetest.register_entity(name, {
		...
		on_step = function(self, dtime)
			...
			if self.type == "monster" and minetest.setting_getbool("only_peaceful_futuremobs") then
				self.object:remove()
			end
...
futuremobs.spawning_futuremobs = {}
function futuremobs:register_spawn(name, nodes, max_light, min_light, chance, active_object_count, max_height, spawn_func)
	futuremobs.spawning_futuremobs[name] = true	
	minetest.register_abm({
		...
		action = function(pos, node, _, active_object_count_wider)
			...
			if not futuremobs.spawning_futuremobs[name] then
				return
			end
			...
			if minetest.setting_getbool("display_mob_spawn") then
				minetest.chat_send_all("[futuremobs] Add "..name.." at "..minetest.pos_to_string(pos))
			end
...
function futuremobs:register_arrow(name, def)
	minetest.register_entity(name, {
		physical = false,
		visual = def.visual,
		visual_size = def.visual_size,
